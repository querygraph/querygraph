...
impl McpServer {
    ...
    fn call_tool(&mut self, params: &Value) -> Result<Value, RpcError> {
        ...
        let result = match name {
            "validate_fihrist" => fihrist::validate_proposal(arguments)
...
fn tool_definitions() -> Value {
    ...
    json!([
        {
            "name": "validate_fihrist",
            "description": "Validate a proposed Fihrist enterprise registry and optionally check compatibility against a previous snapshot. Does not publish or authorize access.",
            "inputSchema": {
                "type": "object", "additionalProperties": false,
                "properties": {
                    "document": string("Fihrist registry JSON text, at most 8 MiB"),
                    "previous": string("Optional previous Fihrist registry JSON text"),
                },
                "required": ["document"],
            },
        },
